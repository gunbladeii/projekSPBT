...
<?php
    ...
    $mem_revenue = $mysqli->query("SELECT * FROM `revenue` WHERE `noIC`='$noIC' AND `month` = '$month' GROUP BY `month`, year");
    $revenue = mysqli_fetch_assoc($mem_revenue);

?>

<!--start if employeeStatus=='temp'-->

            <div class="image"><img src="data:image/jpeg;base64,<?php echo base64_encode($BC['publisherSPBTFacePic']);?>" class="img-fluid img-thumbnail mx-auto d-block" style="width: 80px;height: 80px;border-radius: 15%; vertical-align:middle"/></div>
        ...
        <form method="post" action="payroll.php" role="form" enctype="multipart/form-data">     
        <div class="modal-footer">
        <input type="hidden" name="noIC" value="<?php echo $noIC;?>">
        <input type="hidden" name="grandTotal" value="<?php echo $grandTotal;?>">
        <input type="hidden" name="totalOil" value="<?php echo $totalOil;?>">
        <input type="hidden" name="date" value="<?php date_default_timezone_set("asia/kuala_lumpur"); echo $date2 = date('Y-m-d');?>">
         <input type="hidden" name="month" value="<?php date_default_timezone_set("asia/kuala_lumpur"); echo $month3 = date('m');?>">
        <input type="hidden" name="year" value="<?php date_default_timezone_set("asia/kuala_lumpur"); echo $year = date('Y');?>">
        <?php if($revenue['grandTotal'] == 0 && $revenue['month']){ echo
        '<input type="submit" class="btn btn-primary" name="submit" value="Endorse Payroll"/>';
        } else {echo '<input type="submit" class="btn btn-success" name="update" value="Endorse Payroll"/>';} ?>
